...
export default function AuditLog() {
  ...
  const [filters, setFilters] = useState({ inicio: '2025-01-01', fim: '2025-12-31', acao: '' });
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ padding: 12, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <select className="form-select" value={filters.acao} onChange={e => setFilters({ ...filters, acao: e.target.value })} style={{ width: 130, padding: '6px 10px' }}>
              ...
              <option value="upload">Upload</option><option value="download">Download</option><option value="delete">Exclusão</option>
            </select>
            <button className="btn btn-primary btn-sm" onClick={loadData}>Filtrar</button>
            <button className="btn btn-success btn-sm" style={{ marginLeft: 'auto' }}>⬇ CSV</button>
          </div>
        </div>
        <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', marginBottom: 16 }}>
          <div className="stat-card primary"><div className="label">Total de Ações</div><div className="value">{stats?.total_acoes || logs.length}</div></div>
          ...
          <div className="stat-card accent"><div className="label">Buscas Realizadas</div><div className="value">{stats?.buscas || 0}</div></div>
        </div>
        <div className="card">
